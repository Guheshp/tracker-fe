...
import { Mail, Lock, Eye, EyeOff, ArrowRight, BarChart3, Shield, Zap, TrendingUp } from "lucide-react";
...
export default function Login() {
  ...
  const features = [
    { icon: <Zap className="w-5 h-5" />, title: "Simple Tracking", desc: "One tap daily check-in" },
    { icon: <Shield className="w-5 h-5" />, title: "Secure", desc: "Your data is safe" },
    { icon: <TrendingUp className="w-5 h-5" />, title: "Visual Progress", desc: "Beautiful charts" },
    { icon: <BarChart3 className="w-5 h-5" />, title: "Smart Analytics", desc: "Insights that matter" },
  ];
      ...
      <div className="relative lg:w-1/2 w-full h-full overflow-hidden bg-gradient-to-br from-violet-600 via-purple-600 to-indigo-700 flex items-center justify-center p-8 lg:p-16">
        ...
        <div className="relative z-10 max-w-lg w-full">
          ...
          <p className="text-white/80 text-base lg:text-lg mb-10 leading-relaxed">
            Sign in to continue tracking your habits and building a better version of yourself every single day.
          </p>
          ...
          <div className="grid grid-cols-2 gap-4">
            {features.map((feature, index) => (
              <div
                key={index}
                className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-5 hover:bg-white/15 transition"
              >
                <div className="w-10 h-10 flex items-center justify-center bg-white/20 rounded-xl text-white mb-3">
                  {feature.icon}
                </div>
          ...
          <div className="bg-white border border-gray-200 rounded-3xl p-8 shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="group w-full bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-500 hover:to-purple-500 text-white font-semibold py-3 rounded-xl transition-all shadow-lg shadow-violet-500/30 hover:shadow-violet-500/50 disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {submitting ? (
                  <>
